...
def get_meta_data_from_event(event):
    ...
    return key, metadata['x-amz-meta-uuid'], metadata['x-amz-meta-name'], metadata['x-amz-meta-timestamp']
...
def lambda_handler(event, context):
    ...
    print(f"(1. Metadata from bucket): image key:{key}  name of user:{name} timestamp:{timestamp} user id:{user_id}")
