...
def error(msg):
    return {"status": "ERROR", "message": f"Encountered an Unexpected Error: {msg}"}
...
def lambda_handler(event, context):
    ...
    if "profile" in route:
        ...
        try:
            ...
            print(
                f"(1.1 get profile data): profiles:{profiles}    whitelist:{whitelist}"
            )
            ...
            resp = success(
                f"Successfully retrieved graph data for interval {time_scale}",
                graph_data,
            )
        ...
        try:
            ...
            print(
                f"(1. get detected image data): whitelist:{whitelist} blacklist:{blacklist}"
            )
            detected_images = [whitelist, blacklist]
            labels, datasets, intervals = get_dashboard_template_data(end_date)
            print(
                f"(2. dashboard template data): labels:{labels}   datasets:{datasets}    intervals:{intervals}"
            )
