...
def get_meta_data_from_event(event):
    ...
    return metadata['x-amz-meta-uuid'], key, metadata['x-amz-meta-tag'], metadata['x-amz-meta-camera_id'], metadata[
...
def add_video_to_user_data(event):
    ...
    print(
        f"(1. Get metadata from s3 object): user_id:{user_id};    video key:{video_name};    tag:{tag};    camera_id{camera_id};      timestamp{timestamp}")
    ...
    response = table.update_item(
        ...
        ExpressionAttributeValues={
            ":i": [
                {
                    'aid': hash(f'{video_name}-{user_id}'),
                    # TODO: This hash is debatable...we can decide if it's okay or not 'rr'
                    'metadata': {
                        'camera_id': camera_id,
                        'timestamp': timestamp
                    },
...
def lambda_handler(event, context):
    # invoke the function to add the given video the Artefacts table and correspond with UserData in dynamo
