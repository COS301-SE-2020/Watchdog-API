...
def to_s3(img, key, crop_areas, s3, metadata):
    ...
    for i, crop_area in enumerate(crop_areas):
        ...
        upload_cropped_image = s3.put_object(
            Bucket=os.environ["BUCKET"], Key=_fn, Body=image_bytes, Metadata=metadata
        )
        resp = {
            "filename": _fn,
...
def lambda_handler(event, context):
    ...
    rekognition = boto3.client(
        "rekognition", region_name=os.environ["REKOGNITION_REGION"]
    )
    ...
    if bounding_boxes is not None:
        file_byte_string = s3.get_object(Bucket=os.environ["BUCKET"], Key=key)[
            "Body"
        ].read()
