...
def lambda_handler(event, context):
    ...
    if "/cameras" in route:
        ...
        print(
            f"(1. parameters): site id: {site_id}; old location: {old_loc_name}; new location: {new_loc_name};"
        )
        ...
        if new_loc_name not in site_obj[site_id] and old_loc_name in site_obj[site_id]:
            ...
            try:
                parameters = {
                    ...
                    "ExpressionAttributeValues": {":obj": site_obj[site_id]},
                ...
                response = success(
                    msg=f'Dynamo UpdateItem Completed for "{route}"', extra=response
                )
                print(f"(5. Response): {str(response)}")
                statusCode = 200
            except Exception as e:
                response = error(
                    f"Could not complete Dynamo Operation due to Error: {e}"
                )
                print(str(response))
        else:
            statusCode = 202
            response = error(
                msg="new location already exists for given site, or the old one does not exist"
            )
